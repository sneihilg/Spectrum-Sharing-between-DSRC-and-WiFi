% plotting best response curve for 2 DSRC networks with cost 
clc;
clear all;
close all;
...
M = [1 1 1];                  % number of DSRC nodes in network 1
N = [1 2 5];                  % number of DSRC nodes in network 2
w_col = 1 + beta;
w_idle = beta;
tau_d1 = 1e-2:1e-3:0.99;
tau_d2 = 1e-2:1e-3:0.99;
...
for m = 1:numel(M)
    ...
    for i = 1:numel(tau_d2)
        ...
        [t_d1,fval,exitflag] = fmincon(@(t_d1)optimizing_net_age_cost(beta,t_d1,tau_d2(i),w_col,w_idle,minimum,maximum,M(m),N(m)),t_d_o,[],[],[],[],lb,ub,[],options);
    ...
    for i = 1:numel(tau_d1)
        ...
        minimum = min(age_temp);
        maximum = max(age_temp);
        [t_d2,fval,exitflag] = fmincon(@(t_d2)optimizing_net_age_cost(beta,t_d2,tau_d1(i),w_col,w_idle,minimum,maximum,N(m),M(m)),t_d_o,[],[],[],[],lb,ub,[],options);
    ...
    fig_name = sprintf('M_%d_N_%d_DSRC_cost.pdf',M(m),N(m));
