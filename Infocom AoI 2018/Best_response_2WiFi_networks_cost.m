% plotting best response curve for 2 WiFi networks with cost 
clc;
clear all;
close all;
...
for m = 1:numel(M)
    ...
    for i = 1:numel(tau_w2)
        ...
        lb = 1e-2;
        ub = 0.99;
        [t_w1,fval,exitflag] = fmincon(@(t_w1)optimizing_net_thr_cost(beta,t_w1,tau_w2(i),w_col,w_idle,N(m),M(m)),t_w_o,[],[],[],[],lb,ub,[],options);
    ...
    for i = 1:numel(tau_w1)
        ...
        lb = 1e-2;
        ub = 0.99;
        [t_w2,fval,exitflag] = fmincon(@(t_w2)optimizing_net_thr_cost(beta,t_w2,tau_w1(i),w_col,w_idle,M(m),N(m)),t_w_o,[],[],[],[],lb,ub,[],options);
    ...
    fig_name = sprintf('M_%d_N_%d_WiFi_cost.pdf',M(m),N(m));
