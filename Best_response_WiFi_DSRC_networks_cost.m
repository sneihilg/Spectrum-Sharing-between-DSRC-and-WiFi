% plot the best response curve for age and throughput nodes with cost
clc;
clear all;
close all;
...
for m = 1:numel(M)
    ...
    for i = 1:numel(tau_d)
        ...
        lb = 1e-2;
        ub = 0.99;
        [t_w,fval,exitflag] = fmincon(@(t_w)optimizing_net_thr_cost(beta,t_w,tau_d(i),w_col,w_idle,M(m),N(m)),t_w_o,[],[],[],[],lb,ub,[],options);
    ...
    for i = 1:numel(tau_w)
        ...
        [t_d,fval,exitflag] = fmincon(@(t_d)optimizing_net_age_cost(beta,t_d,tau_w(i),w_col,w_idle,minimum,maximum,M(m),N(m)),t_d_o,[],[],[],[],lb,ub,[],options);
    ...
    fig_name = sprintf('M_%d_N_%d_with_cost_widle_%.3f.pdf',M(m),N(m),w_idle);
    title(sprintf('N_D = %d, N_W = %d',M(m),N(m)),'fontsize',24);
...
for m=1:numel(M)
    ...
    cost = w_idle.*(((1-t_d).^M(m)).*((1-t_w).^N(m))) + w_col.*(1-(((1-t_d).^M(m)).*((1-t_w).^N(m)))...
